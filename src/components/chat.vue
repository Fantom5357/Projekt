<template>
    <div class="homebg text" v-if="username">
        ...
        <div class="chat">
            <h2 class="main">Чат</h2>
            <div class="text main" v-for="message in messages" :key='message.id'>
                {{ message.user }}: {{ message.text }}
            </div>
            <div v-show="emptyMsg" class="empty main">*Пусто*</div>
        </div>
    </div>

    <div class="homebg" v-else>
        <div class="text main">Вернитесь и укажите ник</div>
        <div class="text main">
            <p><router-link :to="{name: 'Home'}"><a class="text">Лааааааааадна....</a></router-link></p>
        </div>
    </div>
</template>
